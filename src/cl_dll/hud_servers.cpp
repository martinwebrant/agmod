...
static int iSortOrder = 1;

#include "agudpclient.h"
#include "parsemsg.h"

// Default master server address in case we can't read any from woncomm.lst file
#define VALVE_MASTER_ADDRESS "hlmaster1.hlauth.net"
...
void CHudServers::QueryThink( void )
{
    ...
    /*
		...
		NET_API->SendRequest( context_id++, NETAPI_REQUEST_DETAILS, 0, 500.0, &p->remote_address, ::ServerResponse );
    ...
    */
    ...
    WORD wPort = htons((WORD)p->remote_address.port);
    char szAddress[64];
    sprintf(szAddress,"%i.%i.%i.%i"
			                ,(int)p->remote_address.ip[0]
			                ,(int)p->remote_address.ip[1]
			                ,(int)p->remote_address.ip[2]
			                ,(int)p->remote_address.ip[3]);
    AgUDPClient Server;
    if (Server.Connect(szAddress,wPort))
    { 
      const char Details[] = { '\xff', '\xff', '\xff', '\xff', 'd', 'e', 't', 'a', 'i', 'l', 's', '\n' };
      int  SizeDetails = sizeof(Details);
      int iPing = GetTickCount();
      Server.Send(Details,SizeDetails);
      if (Server.WaitForRecieve(500))
      {
        char szBuffer[4000];
        int iBytes = Server.Receive(szBuffer,sizeof(szBuffer));
        iPing = GetTickCount() - iPing;
        if (iBytes > 10)
        {
          char szResponse[512];
		  char szGameDir[32];
          int iSize = 0;
          BEGIN_READ(szBuffer,iBytes);
          READ_LONG();        //Header
          READ_BYTE();        //Responce
          READ_STRING();      //Address
          iSize += sprintf(&szResponse[iSize],"hostname\\%s",READ_STRING());
          iSize += sprintf(&szResponse[iSize],"\\map\\%s",READ_STRING());
          strcpy(szGameDir,READ_STRING()); //Game dir
          iSize += sprintf(&szResponse[iSize],"\\desc\\%s",READ_STRING());
          iSize += sprintf(&szResponse[iSize],"\\current\\%d",(int)READ_BYTE());
          iSize += sprintf(&szResponse[iSize],"\\max\\%d",(int)READ_BYTE());
          int iProtocol = (int)READ_BYTE();
          READ_BYTE(); //Type
          READ_BYTE(); //os
          iSize += sprintf(&szResponse[iSize],"\\pass\\%d",(int)READ_BYTE());
          if (iProtocol >= 45)
          {
		        // Setup context_id
		        p->context = context_id;

            net_response_t* response = new net_response_t;
            response->response = szResponse;
            response->ping = iPing / 1000.0;
            response->context = context_id;
            response->type = NETAPI_REQUEST_DETAILS;
            response->error = 0;
            memcpy(&response->remote_address,&p->remote_address,sizeof(p->remote_address));
			...
			const char AgDetails[] = { '\xff', '\xff', '\xff', '\xff', 'a', 'g', 'd', 'e', 't', 'a', 'i', 'l', 's', '\n' };
			int  AgSizeDetails = sizeof(AgDetails);
			Server.Send(AgDetails,AgSizeDetails);
			if (Server.WaitForRecieve(500))
			{
				char szBuffer[4000];
				int iBytes = Server.Receive(szBuffer,sizeof(szBuffer));
				if (iBytes > 5)
				{
					BEGIN_READ(szBuffer,iBytes);
					READ_LONG();        //Header
					iSize += sprintf(&szResponse[iSize],"\\%s",READ_STRING());
				}
			}
...
void CHudServers::RequestList( void )
{
  ...
  /*
	NET_API->SendRequest( context_id++, NETAPI_REQUEST_SERVERLIST, 0, 30.0, &adr, ::ListResponse );
  */

  AgUDPClient Masters;
  char szBuffer[8192];
  WORD wPort = htons((WORD)adr.port);
  char szAddress[64];
  sprintf(szAddress,"%i.%i.%i.%i"
			              ,(int)adr.ip[0]
			              ,(int)adr.ip[1]
  ...
  if (Masters.Connect(szAddress,wPort))
  {
    static char Master[] = { '1', '0','0','0','0', '\\','g','a','m','e','d','i','r','\\','%','s','\n', '\0'};
    char szMaster[64];
//	sprintf(szMaster,Master,"ag");
    sprintf(szMaster,Master,AgGetGame());
    int  iSizeMaster = strlen(szMaster);
    ...
    while (iUnique > 0)
    {
      Masters.Send(szMaster,iSizeMaster);

      if (Masters.WaitForRecieve(2000))
      {
        int iBytes = Masters.Receive(szBuffer,sizeof(szBuffer));
        if (iBytes > 10)
        {
          BEGIN_READ(szBuffer,iBytes);
          int iHeader       = READ_LONG();
          int iResponse     = READ_BYTE();
          int iUnused       = READ_BYTE();
          iUnique           = READ_LONG();
          *(int*)&szMaster[1] = iUnique;

	        // Each address is 6 bytes long
	        int iNumAddresses = (iBytes - 4 - 1 - 1 -4) / 6;
	        while (iNumAddresses-- > 0)
	        {
				    request_t *p = new request_t;
		        for (int i = 0; i < 4; i++)
			        p->remote_address.ip[i] = READ_BYTE();
            p->remote_address.port = READ_WORD();
            p->remote_address.type = NA_IP;
